...
def authenticate(identifier: str, password: str):
    ...
    email_ident = normalize_email(identifier) or ident
    acct = query(
...
def register_account(first_name: str, last_name: str, email: str, password: str, role: str = "user"):
    ...
    cur = execute(
        "INSERT INTO accounts(username, display_name, first_name, last_name, email, password_hash, role, is_active, must_change_password) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0)",
...
def init_auth(app) -> None:
    ...
    def _csrf_and_timeout():
        ...
        if acct:
            sync_password_change_notification(acct)
            session["must_change_password"] = bool(acct["must_change_password"])
